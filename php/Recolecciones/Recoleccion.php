...
class Recoleccion{
	...
	public function busquedaNombre($nombre){
		...
		$conexion = $this->conn;
		$nombre = '%'.$nombre.'%';
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' where nombre like :nombre and estado = 1 and estatus = 1  ORDER BY id_form ', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
	...
	public function busquedaUltimos(){
		...
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.'  where estado = 1 and estatus = 1 ORDER BY id_form DESC LIMIT 15', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$sth->execute();
		$row = $sth->fetchAll(PDO::FETCH_ASSOC);
		return $row;
	}
	...
	public function busquedaAnualidad($ano){
		...
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  year(fecha_form) = :ano  and estado = 1 and estatus = 1 ORDER BY id_form  ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
	...
	public function busquedaMesualidad($mes){
		...
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  month(fecha_form) = :mes    and estado = 1 and estatus = 1   ORDER BY id_form   ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
	...
	public function BusquedaEspecifica($fecha_inicio , $fechaFinal){
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  fecha_form BETWEEN :fechai and :fechaf   and estado = 1 and estatus = 1  ORDER BY id_form   ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
	...
	public function busquedaUltimosCompletadas(){
	
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.'  where estado = 1 and estatus = 0 ORDER BY id_form DESC LIMIT 15', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$sth->execute();
		$row = $sth->fetchAll(PDO::FETCH_ASSOC);
		return $row;
	}
	...
	public function busquedaAnualidadCompleto($ano){
	
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  year(fecha_form) = :ano  and estado = 1 and estatus = 0 ORDER BY id_form  ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$sth->bindParam(':ano', $ano, PDO::PARAM_STR );
		$sth->execute();
		$row = $sth->fetchAll(PDO::FETCH_ASSOC);
		return $row;
	}
	public function busquedaMesualidadCompleto($mes){
	
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  month(fecha_form) = :mes    and estado = 1 and estatus = 0  ORDER BY id_form  ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$sth->bindParam(':mes', $mes, PDO::PARAM_STR );
		$sth->execute();
		$row = $sth->fetchAll(PDO::FETCH_ASSOC);
		return $row;
	}
	public function BusquedaEspecificaCompleta($fecha_inicio , $fechaFinal){
		$conexion = $this->conn;
		$sth = $conexion->prepare('SELECT id_form, nombre , fecha_form , longitud , latitud, comentarios ,telefono from '.$this->nombreTabla.' WHERE  fecha_form BETWEEN :fechai and :fechaf   and estado = 1 and estatus = 0 ORDER BY id_form  ASC', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$sth->bindParam(':fechai', $fecha_inicio , PDO::PARAM_STR );
		$sth->bindParam(':fechaf',  $fechaFinal, PDO::PARAM_STR );
		$sth->execute();
		$row = $sth->fetchAll(PDO::FETCH_ASSOC);
