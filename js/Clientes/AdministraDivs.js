...
function decode_utf8(s) {
  return decodeURIComponent(escape(s));
		...
		//Agregamos el formulario de alta default
		 $('#panelBody').empty();
		 var contenido ='<form id="formularioRegistroCli"  class="controlPanel hideElement"> <label id="estatusRegistro"></label> <div class="form-group"> <label for="nombre">Nombre del cliente:</label> <input type="text" class="form-control" maxlength="50" placeholder="Ingrese el nombre del cliente" autofocus required name="nombre_cliente"> </div> <div class="form-group"> <label for="nombre">RFC del Cliente:</label> <input type="text" class="form-control" maxlength="20" placeholder="Ingrese el RFC del cliente" autofocus required name="rfc"> </div> <div class="form-group"> <label for="nombre">URL de imagen de perfil:</label> <input type="text" class="form-control" maxlength="150" placeholder="Ingrese url de imagen de perfil" autofocus name="url_image"> </div> <div class="form-group"> <label for="tel">Telefono del cliente:</label> <input type="tel" class="form-control" maxlength="15" placeholder="Ingrese el telefono del cliente." name="telefono"> </div> <div class="form-group"> <label for="email">Correo electronico:</label> <input type="email" class="form-control" id="email" maxlength="40" placeholder="Ingrese algun correo electronico." required name="email"> <label id="validoEmailForm"></label> </div> <div class="form-group"> <label for="text">Nickname del cliente:</label> <input type="text" class="form-control" id="nickname" maxlength="15" placeholder="nickname del cliente" required name="nickname"> <label id="validoNickname"></label> </div> <div class="form-group"> <label for="psw">Contraseña:</label> <input type="password" class="form-control" maxlength="20" placeholder="Contraseña del proveedor." required name="password"> </div> <div class="form-group resetDisplay"> <label for="sel1">Tipo de Cliente:</label> <select class="form-control" id="list1" name="listatipo"> <option value="1">Comercial</option> <option value="2">Industrial</option> </select> </div> <br> <button type="submit" id="botonFormularioRegistro" class="btn btn-default" disabled>Registra al cliente</button> </form>';
		  $('#panelBody').append( contenido );
		 $('#formularioRegistroCli').slideDown("slow");
	...
	$(document).on('submit','#formularioRegistroCli', function(e){
		 e.preventDefault();
		  var values = JSON.stringify($('#formularioRegistroCli').serializeObject());
		   console.log(values);
                    ...
                    success: function(data){
                     resultado = data;
                      if(resultado['respuesta'] == 'El nickname es valido en el formulario.' && (largo>=6)){
                        ...
                        $('#validoNickname').addClass('label label-info');
                      }
                      else{
                        if(largo<6){
...
function busquedaEspecifica(selector , valorCaja1 ,valorCaja2){
	...
	for(var item in data) {
	                            resultado+='<tr><td><label  class="form-control table-striped table-hover visualizacion">';	
					 			resultado+=data[item]['id_Cliente'];
					 			resultado+='</label></td>';	
	                            resultado+= '<td>'+'Nombre: '+ data[item]['nombre_Cliente'] +'<hr>';
					 			...
					 			resultado+='<td><button type="button" class="btn btn-warning" value = "0" role="editar">Editar Cliente</button>';	
					 				 
					 			resultado+='</td><td>';	
					 			if(data[item]['id_Cliente']!=1){
					 			resultado+='<button type="button" class="btn btn-danger" value = "0" role="eliminar" data-toggle="modal" data-target="#modalDeletes">Eliminar Cliente</button>';	
					 			}
