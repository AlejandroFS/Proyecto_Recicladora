...
$(document).ready(function(){
	...
	function obtenerDatoscliente(){
		//Obtenemos el id del cliente
			//---->Este ajax funciona para sacar el id del cliente de una orden de compra, la fecha y el numero de orden
			//---->El segundo ajax funciona para sacar los datos del cliente
		$.ajax({
						              type: "POST",
						              dataType: "json",
						              url: MostrarOrdenes ,  
						              ...
						              var id_clienteLocal = data['id_cliente'];
						              fecha_orden = data['fecha'];
						            
						              //console.log(id_clienteLocal+'Numero de cliente');
						               //console.log(variableGlobalId +'Numero de orden');
						              $.ajax({
						              type: "POST",
						              dataType: "json",    //
						              url: MostrarClientes,
						              data: {muestra: "bid",id_Cliente: id_clienteLocal },
						              success:function(data){
						            	  console.log(data);
						              objetocliente = data;
						              mostrarOrden();
	...
	function busquedaDesechos(){
		 ...
		 $.ajax({
						             type: "POST",
						              ...
						              var valor = elemento['precio_kg_Venta'];
						            ...
						            var i = 1;
						             for(var item in data) {
						              ...
						              // console.log(elemento);
						               ...
						               contenido +='<td>'+ elemento['Precio_kg_Venta']+'</td>';
						             
						               var precioRow = elemento['Precio_kg_Venta'] * elemento['totalkg'];
						               total +=  precioRow ;
						               contenido +='<td> $'+ precioRow +'</td>';	 	       
						                  contenido +='<td>'+'<a value="'+elemento['id_venta']+'"class="btn btn-danger">Eliminar</a>'+'</td></tr>';  
						                  i++; 
						              }
		...
		$('#comment').empty();
			$.ajax({
						             type: "POST",
						              dataType: "json",
						              url: MostrarOrdenes,
						              data: { id_ordenVenta: variableGlobalId} ,
						              success:function(data){
						              console.log(data);
...
if($(this).attr('role')=='editar'){   
            ...
            var arrayData = ['id_orden','observaciones','fecha','nombre_cliente','fecha'];
            var obj = {};
            var arrayValues = [];
                    for (var i = 0; i < 6; i++) {
                    ...
                    varGlobalcliente = valores_editados['id_cliente'];
        			console.log(varGlobalcliente+'--<' );
...
//Funcionamiento para guardar las items en la orden(Panel de control)
 $(document).on('click','#botonAgregarItem', function(e){
	 ...
	 var datos = {total_Kg: Number($('#totalKg').text()), id_Material: id_material, precio_kg_venta : Number($('#precioKg').text()),id_orden: variableGlobalId};
	 console.log(datos);
$.ajax({
				             type: "POST",
				              dataType: "json",
